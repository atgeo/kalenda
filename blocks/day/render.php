...
?>
<div <?php echo get_block_wrapper_attributes( array( 'class' => 'mcc-day--' . $my_catholic_calendar_style ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- core-escaped. ?>>
	<div class="mcc-day__header">
	<h2 class="mcc-day__title">
		<?php echo esc_html( $my_catholic_calendar_title ); ?>
	</h2>
	<?php if ( $my_catholic_calendar_show_date ) : ?>
		<p class="mcc-day__date">
			...
			<ul class="mcc-day__events-list">
				<?php foreach ( $my_catholic_calendar_events as $my_catholic_calendar_event ) : ?>
					<li class="mcc-day__event">
						...
						?>
						<h3 class="mcc-day__name event-color-<?php echo esc_attr( (string) ( $my_catholic_calendar_event['color'][0] ?? 'white' ) ); ?>">
